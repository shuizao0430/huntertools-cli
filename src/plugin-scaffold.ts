 ...
 *
 * Usage: huntertools plugin create <name> [--dir <path>]
 *
 * Creates:
 *   <name>/
...
export function createPluginScaffold(name: string, opts: ScaffoldOptions = {}): ScaffoldResult {
  ...
  const manifest = {
    name,
    version: '0.1.0',
    description: opts.description ?? `A HunterToolsCLI plugin: ${name}`,
    opencli: `>=${PKG_VERSION}`,
  };
  ...
  // package.json
  const pkg = {
    name: `huntertools-plugin-${name}`,
    version: '0.1.0',
    type: 'module',
    description: opts.description ?? `A HunterToolsCLI plugin: ${name}`,
    peerDependencies: {
      huntertoolscli: `>=${PKG_VERSION}`,
    },
  };
...
# See: https://github.com/shuizao0430/huntertools-cli
...
import { cli, Strategy } from 'huntertoolscli/registry';
...
`;
  ...
  // README.md
  const readme = `# huntertools-plugin-${name}

${opts.description ?? `A HunterToolsCLI plugin: ${name}`}

## Install

\`\`\`bash
# From local development directory
huntertools plugin install file://${targetDir}

# From GitHub (after publishing)
huntertools plugin install github:<user>/huntertools-plugin-${name}
\`\`\`
...
huntertools list | grep ${name}

# Run a command
huntertools ${name} hello
huntertools ${name} greet --name World
\`\`\`
`;
